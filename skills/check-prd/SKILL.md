...
2. Determine and record:
   ...
   - 是否涉及 AI 功能
3. **判断文档级别（L1-L4）**，并在评审开头声明，避免用错误标准评审：
   ...
   是否新增页面、新增实体、新增角色/权限？ → 是 → L3（模块级）
           ↓ 否
   是否修改/新增业务规则？ → 是 → L2（规则级）
           ↓ 否
   仅配置/文案/样式/阈值变更 → L1（配置级）
   ```

   在评审开头输出：
   > 该文档为 **L{X}（{级别名}）** 需求文档，以下维度不适用：{列出跳过的维度}

4. Mark non-applicable dimensions before starting the ordered review.

### Level-based Applicability Rules

| 文档级别 | 不适用的维度 | 说明 |
| --- | --- | --- |
| L1（配置级） | 06 架构、07 数据建模、08 流程与角色（完整版）、10 商业分析、11 MVP、14 运营方案 | 仅检查：变更说明、边界处理、验收标准 |
| L2（规则级） | 06 架构、07 数据建模、10 商业分析、11 MVP | 重点检查：规则完整性、边界场景覆盖度、交互变更描述 |
| L3（模块级） | 10 商业分析（可选）、11 MVP 完整版 | 标准评审，但架构层只检查 6.1-6.4 |
| L4（系统级） | 全量评审 | — |

### Product-type Applicability Rules
